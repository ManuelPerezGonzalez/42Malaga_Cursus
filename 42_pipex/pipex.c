...
void    ft_child(int *end, char **argv, char **envp)
{
    ...
    close(end[0]); // Child closes cmd2 stdin, we don't need it.
    ...
    close(end[1]);
    args_execve = ft_split(argv[2], ' '); // Save every cmd1 order in a list.
    path = ft_get_path(args_execve[0], envp); // Using environment PATHS, check which is correct.
...
void    ft_parent(int *end, char **argv, char **envp)
{
    ...
    // Open outfile in read-only mode, if it already exists, resets it to 0 and if it doens't, create it (returns int).
    ft_check_end(fd_file); // Check for error.
    dup2(end[1], STDIN_FILENO); // Duplicate cmd2 stdin.
    close(end[1]);
    dup2(fd_file, STDOUT_FILENO); // Duplicate outfiles fd.
    close(fd_file);
    args_execve = ft_split(argv[3], ' '); // Save every cmd2 order in a list.
    path = ft_get_path(args_execve[0], envp); // Using environment PATHS, check which is correct.
    if (execve(path, args_execve, envp) == -1)
	{
		perror("Pipex: command not found");
...
void    pipex(char **argv, char **envp)
{
    ...
    if (pid < 0) // When -1 is error.
        ...
        child_process(end, argv, envp); // Defines what to do when in child process.
    else // When other number is parent process.
    {
        pid = fork();
        if (pid < 0)
            perror("Fork: ");
        else if (!pid) // This time we'll treat 0 as parent...
        parent_process(end, argv, envp); // Defines what to do when in parent process.
        else
